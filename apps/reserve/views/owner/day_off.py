...
class DayOffCreateView(views.APIView):
    serializer_class = DayOffCreateSerializer
...
class DayOffListView(views.APIView):
    ...
    def get(self, request):
        ...
        days_off = (
            DayOff.objects.filter(market_access_filter('market__', request.user))
            .distinct()
...
class DayOffDeleteView(views.APIView):
    ...
    def delete(self, request, pk):
        day_off = get_object_or_404(
            ...
            id=pk,
            market__in=accessible_markets(request.user, write=True),
        )
        day_off.delete()
